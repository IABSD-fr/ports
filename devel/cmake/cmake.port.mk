BUILD_DEPENDS+=	devel/cmake/core>=4
...
MODCMAKE_configure =	for f in $$(find ${WRKSRC} -name CMakeLists.txt -o -name '*.cmake'); \
	do \
		sed -i -e 's/STREQUAL "OpenBSD"/MATCHES "OpenBSD|IABSD"/g' \
			-e 's/STREQUAL OpenBSD)/MATCHES "OpenBSD|IABSD")/g' \
...
.if ${MODCMAKE_POLICY_VERSION_OVERRIDE:L} != no
.  if ${MODCMAKE_POLICY_VERSION_OVERRIDE:L} == yes
CONFIGURE_ARGS += -DCMAKE_POLICY_VERSION_MINIMUM=${MODCMAKE_POLICY_VERSION_OVERRIDE_VER}
.  endif
.endif
